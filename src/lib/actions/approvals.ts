...
// backend, so a request created on web can be decided from mobile and vice
// versa — decideApprovalRequest below mirrors web's ApprovalsClient.tsx
// exactly, including applying the underlying change under the ORIGINAL
// requester's attribution (not the approving manager's), via the same
// performRecordSale/performDeleteSale/performUpdateProduct split web uses.
...
async function requireApproverProfile(): Promise<Profile> {
  const profile = await requireProfile();
...
async function loadRequesterCtx(requestedBy: string, orgId: string) {
  const { data: requester, error } = await supabase.from('profiles').select('role, first_name, last_name').eq('id', requestedBy).single();
  if (error || !requester) throw new Error("The original requester's profile could not be found.");
  return { orgId, userId: requestedBy, role: requester.role, actorName: `${requester.first_name} ${requester.last_name}` };
}
...
export async function decideApprovalRequest(requestId: string, decision: 'approved' | 'rejected', rejectReason?: string): Promise<void> {
  ...
  if (decision === 'rejected') {
    ...
    await notifyRequester({ org_id: request.org_id, requested_by: request.requested_by, entity_type: request.entity_type, id: requestId }, 'rejected', rejectReason);
    void logAudit({
  ...
  const requesterCtx = await loadRequesterCtx(request.requested_by, request.org_id);
  ...
  await notifyRequester({ org_id: request.org_id, requested_by: request.requested_by, entity_type: request.entity_type, id: requestId }, 'approved');
  void logAudit({
    orgId: profile.org_id,
    actorId: profile.id,
    actorName: `${profile.first_name} ${profile.last_name}`,
